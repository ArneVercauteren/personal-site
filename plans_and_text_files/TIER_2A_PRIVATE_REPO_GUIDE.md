...
| `universe` | The stocks the strategy may choose from. Leave it `[]` to use the shared self-refreshing universe (`public/data/universe.json`, rebuilt monthly) — recommended for kings so they stay current. Set an explicit list only to pin a fixed basket. |
...
import pandas as pd
from paper_trading import portfolio, prices, universe        # borrowed from the public repo
...
def run():
    ...
    for path in sorted((ROOT / "strategies").glob("*.json")):
        ...
        tickers = universe.resolve_universe(spec)   # explicit, else the shared universe
